...
Data KdTree::search(KdNode *node, QVector3D pos,bool *mask)
{
    ...
    while(q!= node)
    {
        ...
        if(fabs(pos[split]-q->data.value[split]) <(minDis))
            /* If the circle (sphere or hypersphere) with data as the center, the radius is minDisThe circle of intersects the segmentation hyperplane,
              then jump to the other subspace to search*/
        {
            Data tmpResult;
            if(tmp == q->left)
            {
