...
public:
    ...
    vec3(Point a,Point b){ x = b.x-a.x; y = b.y-a.y; z = b.z-a.z; }  //The vector from a to b
    void init(float x1,float y1,float z1=0){x = x1;y = y1;z = z1;}
...
struct Line{
    Point p1,p2; //Directed line segment, p1 points to p2
    Line(){}
...
public:
    ...
    static float PointToLine(Point p,Line l)
    {
        vec3 ab(l.p1,l.p2);
        vec3 ac(l.p1,p);
        return Norm(Cross(ab,ac))/PointToPoint(l.p1,l.p2); //d = (AB x AC)/|AB| ,|AB X AC|/2
        //Is the area of ​​triangle ABC, and the base of this triangle is |AB|，Height is the distance from C to AB
    }
    ...
    //The angle between two space vectors
    ...
    static bool IsRightPoint(vec3 pt, Line line)
    {
        ...
        if(tmp>0) //Greater than 0 on the right
        {
            return true;
        }else
        {
...
struct Triangle{
    Point p1,p2,p3;//Three dots
    Point p[3];
    Line l1,l2,l3;//Three lines
    Line l[3];//Three lines
    Triangle(){}
        ...
        if(left<right){
            ...
            while(i<j)
            {
                while(i<j&& cmp(temp,a[j]))
                    j--;
                if(j>i)
                    a[i++]=a[j];//a[i] Has been assigned to temp, so directly assign a[j] to a[i],After assignment a[j],Available
                while(i<j&&cmp(a[i],temp))
                    i++;
                if(i<j)
                    a[j--]=a[i];
            }
            a[i]=temp;//Insert the benchmark, at this time i and j are equal R[low..pivotpos-1].keys≤R[pivotpos].key≤R[pivotpos+1..high].keys
            quickSort(a,left,i-1,cmp);//Recursive left
            quickSort(a,i+1,right,cmp);//Recursive right
        }
    }
