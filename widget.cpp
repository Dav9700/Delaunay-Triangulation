    ...
    , ui(new Ui::Widget)
{
    ...
    this->setWindowTitle("Delaunay Triangulation_by_DG");
    drawType =3;
    isShowColor = false;
    drawTag =0;
    imageType = -1;  //Negative one means no picture is imported
    isGenTriImg =false;
...
void Widget::paintEvent(QPaintEvent *e)
{
    ...
    if(drawTag == 0){
        ...
        if(drawType == 1|| drawType == 2||drawType == 3){
            ...
            for(int i =0; i< mv.size(); i++) //Convex hull
            {
                if(i == mv.size()-1)
        ...
        if(drawType == 2 ){
            ...
            QList<Triangle> m_tri = hullTins;  //Draw triangles
        ...
        if(drawType == 3){
            ...
            p.setPen(pen);
            p.setBrush(QColor(255,255,255));
            QList<Triangle> m_tri = DelaTins;  //Draw triangles
            for(int i = 0; i< m_tri.size(); i++)
            {
                Triangle tri = m_tri[i];
                QPointF tripts[3];
    ...
    if(drawTag == 1)
    {
        ...
        if(imageType == 1)
        {
            if(isGenTriImg == false)
            {
                ...
                QList<Triangle> m_tri = ImageTins;  //Draw triangles
                for(int i = 0; i< m_tri.size(); i++)
                {
                    Triangle tri = m_tri[i];
                    QPointF tripts[3];
...
void Widget::on_colorBtn_clicked()
{
    if(isShowColor ==false){
        drawType = 4;
        ui->colorBtn->setText(QString("Removed"));
    }else
    {
        drawType = 3;
        ui->colorBtn->setText(QString("Generated color"));
    }
...
void Widget::on_triBtn_clicked()
{
    if(imageType!= -1)
    {
        ...
        bool* maskPoint=new bool[points1.size()];
...
//                }
        ...
        bool* mask=new bool[points.size()];
